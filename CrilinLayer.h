...
#ifdef converter_cxx
converter::converter(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      string fname = "/lustre/cmsdata/fnardi/MuColl_sim/"+std::to_string(G4energy)+"GeV_1.57rad/photonGun_"+std::to_string(G4energy)+"GeV_1.57rad_gen_crilin_v2.root";
...
Int_t converter::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t converter::LoadTree(Long64_t entry)
{
...
void converter::Init(TTree *tree)
{
...
void converter::Show(Long64_t entry)
{
...
Int_t converter::Cut(Long64_t entry)
{
